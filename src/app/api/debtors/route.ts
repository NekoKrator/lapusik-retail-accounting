...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!name || typeof name !== 'string' || name.trim().length === 0) {
      return NextResponse.json({ error: 'Invalid debtor name' }, { status: 400 });
    }
    ...
    if (existing) {
      const updated = await prisma.debtor.update({
        ...
        data: { amount: existing.amount + amount, updatedAt: new Date() },
      });
    ...
    return NextResponse.json({ error: 'Failed to create or update debtor' }, { status: 500 });
  }
}