...
import { expenseJoinedColumns } from "@/db/joined-columns";
...
import { ExpenseStatsBodySchema } from "@/schemas/expense/expense-stats-payload";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validate = validateRequest({
      bodySchema: ExpenseStatsBodySchema,
    });
    ...
    const whereSQL = buildWhereSQL(where, expenseJoinedColumns);
    ...
    const items = await prisma.$queryRaw<ExpenseStats[]>`
      SELECT
				e.id,
        e."category",
				e."amount",
