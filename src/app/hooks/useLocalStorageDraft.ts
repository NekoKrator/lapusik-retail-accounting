...
export function useLocalStorageDraft() {
  ...
  const loadDraft = useCallback(() => {
    ...
    try {
      ...
      if (parsed.additionalBalances) {
        console.log('Setting additionalBalances:', parsed.additionalBalances);
        setAdditionalBalances(parsed.additionalBalances);
      }
      if (typeof parsed.totalCashRegister === 'number') {
        console.log('Setting totalCashRegister:', parsed.totalCashRegister);
        setTotalCashRegister(parsed.totalCashRegister);
      }
      if (parsed.actualEveningBalance) {
        console.log('Setting actualEveningBalance:', parsed.actualEveningBalance);
  ...
  const saveDraft = useCallback(() => {
    if (typeof window === 'undefined' || isInitialLoad.current || !isMounted.current) {
      console.log('Skipping save - window undefined, initial load, or not mounted');
      return;
    }
  ...
  }, [additionalBalances, totalCashRegister, actualEveningBalance, expenseItems]);
