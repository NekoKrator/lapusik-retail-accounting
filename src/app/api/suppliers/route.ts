...
async function getPreviousDayBalance(storeId: string, date: Date) {
  ...
  return previousReport?.actualEveningBalance || previousReport?.calculatedEveningBalance || 0;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const suggestedMorningBalance = await getPreviousDayBalance(data.storeId, new Date(data.date));

    // calculate the total amount of expenses
    const totalExpenses =
      data.breakdown.terminalExpenses +
      data.breakdown.ownerWithdrawal +
      data.breakdown.rent +
      data.breakdown.utilities +
      data.breakdown.supplierPayments +
      data.breakdown.salaries +
      data.breakdown.piggyBank +
      (data.breakdown.otherExpenses || 0);
    ...
    return NextResponse.json({ newReport, suggestedMorningBalance, totalAvailable }, { status: 201 });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!reportId || actualEveningBalance === undefined) {
      return NextResponse.json({ error: 'reportId and actualEveningBalance are required' }, { status: 400 });
    }
    ...
    const difference = report.calculatedEveningBalance - actualEveningBalance
