...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      return NextResponse.json({ error: 'Missing or invalid balances' }, { status: 400 });
    }
    ...
    const suggestedMorningBalance = await getPreviousDayBalance(data.userId, new Date(data.date));
